...
class InMemoryExchangeRateRepository(ExchangeRateRepository):
    ...
    def _rate_key(
        self, user_id: str, exchange_rate: ExchangeRate
    ) -> tuple[str, str, str]:
        """Build storage key from user id and exchange rate currencies."""
        return self._pair_key(
            user_id, exchange_rate.source_currency, exchange_rate.target_currency
        )
    ...
    def _pair_key(
        ...
        """Build storage key from user id and explicit currency codes."""
        return (user_id, source_currency.value, target_currency.value)
